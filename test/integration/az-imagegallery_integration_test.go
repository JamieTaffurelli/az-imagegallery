...
func TestIT_OutputsAreCorrect(t *testing.T) {
	...
	assert.Equal(t, terraform.Output(t, tfOptions, "user_assigned_identity_id"), "/subscriptions/"+os.Getenv("AZURE_SUBSCRIPTION_ID")+"/resourceGroups/testrg/Microsoft.ManagedIdentity/userAssignedIdentities/imggal", "User Assigned Identity Resource ID should be correct")
}
